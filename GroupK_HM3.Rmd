...
By providing `df` and not specific knot locations, the bs() function places knots at quantiles of the predictor variable (`dis`) by default, this ensures that the knots are distributed according to the data density.
...
The best value for degrees of freedom is df = 5, as it has the lowest cross-validation error. The CV error decreases from df = 3 to df = 5, showing that increasing flexibility improves the model's performance up to this point. After df = 5, the CV error either increases or remains nearly the same, suggesting that adding more flexibility does not significantly improve the model. This indicates potential overfitting at higher degrees of freedom.
...
By ordering of the models based on their results (GCV, Deviance Explained and Adjusted \( R^2 \)) we have: 
...
The results highlight the superiority of spline-based models over traditional polynomial fits in capturing complex relationships. The GAM with GC emerges as the best overall choice due to its ability to balance flexibility and generalizability, as indicated by its low GCV and high adjusted. The unpenalized TPS and CRS models provide strong fits but suffer from higher GCV values, suggesting they may overfit the data. Meanwhile, the polynomial fit lags behind in both fit quality and predictive performance, demonstrating the limitations of relying on simpler parametric approaches for this data.
